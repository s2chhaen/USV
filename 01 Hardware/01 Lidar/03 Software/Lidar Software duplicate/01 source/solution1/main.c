...
bool dataReceive(uint8_t adress, uint8_t data[], uint8_t length){
	...
	uint8_t configL = config&0xF;
	uint8_t configH = (config&0xF0)>>8;
...
void setup() {
	uint8_t start_command[] = {0x02, 0x80, 0x02, 0x00, 0x20, 0x24, 0x34, 0x08};
	init_Core_CLK();
...
uint8_t init(){
	...
	USART_set_Bytes_to_receive(iUSART1,1);
	USART_send_Array(iUSART0, SENSOR_ADR, start_command, sizeof(start_command));
	return 0;
}
