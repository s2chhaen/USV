...
uint8_t initDev(usvMonitorHandler_t* dev_p, usvMonitorConfig_t* config_p){
	...
	dev_p->statusObj.uart = config_p->usartNo;
	bool devInit = USART_init(config_p->usartNo,config_p->baudrate, config_p->bits, config_p->parity, config_p->stopbit, config_p->sync, config_p->MPCM,config_p->address, config_p->PortMux);
	USART_set_send_Array_callback_fnc(dev_p->statusObj.uart,config_p->tx_p);
	USART_set_Bytes_to_receive(dev_p->statusObj.uart,MAX_BYTE_SEND);
	USART_set_receive_Array_callback_fnc(dev_p->statusObj.uart,config_p->rx_p);
	dev_p->statusObj.initState = (!devInit)?1:0;
