function val = printOutFile(type,filterName,value)
    ...
    if strcmp(type,'fixed')
        path = strcat('../FilterTesting/FilterTesting/output_',filterName,'_fixed.txt');
        ...
        for i=1:numel(value)
            fixedPoint(i) = floor(value(i)*2^fractionLen);
        end
        %Schreiben der Daten in "input_fixed.txt"
        fprintf(fileID,'%d \n',fixedPoint);
        %Schließen der Datei
    elseif strcmp(type,'float')
        path = strcat('../FilterTesting/FilterTesting/output_',filterName,'_float.txt');
        fileID = fopen(path,'w+','l','UTF-8');
        %Schreiben der Daten in "input_fixed.txt"
        fprintf(fileID,'%7.4f \n',value);
        %Schließen der Datei
