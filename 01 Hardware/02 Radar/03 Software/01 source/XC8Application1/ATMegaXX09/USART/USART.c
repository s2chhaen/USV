...
uint8_t USART_end_of_string[4] = {0};																		// 4 Bytes		//im Vgl. zu Bib-Datei hinzugefuegt von Haendel, 13.03.2023 
	...
	else // normale USART Operation
	{
		...
		if( fastSTATUS(handle).Register.RXCIF == true )
		{
			...
			if(FIFO_chk_for_writedata(&USART_RX_FIFO[number]))
			{
				...
				/*		Hier erfolgt eine Anpassung der UART-Uebertragung aus der Bibliothek, da nicht immer eine vordefinierte Bytezahl uebertragen werden soll, sondern bis zum Zeichenkettenende */
				// Pruefe ob die vorgegebene Anzahl an Bytes gelesen worden sind, wenn die Anzahl definiert wurde oder ob Zeichenketten-Ende erreicht ist. Meist Linefeed 0x0A	
				if(((USART_bytes_to_receive[number]!=0) && (USART_bytes_to_receive[number] == USART_bytes_received[number])) || (Buffer==USART_end_of_string[number]))	//geaendert im Vgl. zu Bib-Datei; Haendel, 13.03.2023
				{
					#if USART_USE_EVENTSYSTEM==1
...
// Die Framelänge wird mit der Funktion "USART_set_Bytes_to_receive" eingestellt.
//
//	Übernahmeparameter:
//		uint8_t USARTnumber -> Nummer des USART-Ports
...
////////////////////////////////////////////////////////////////////////////////////////////////////
// void USART_set_end_of_string(uint8_t USARTnumber, uint8_t end_of_string)			//im Vgl. zu Bib-Datei hinzugefuegt von Haendel, 13.03.2023 
//
// Legt das Zeichenkettenende der Uebertragung fest
//
//	Übernahmeparameter:
//		uint8_t USARTnumber -> Nummer des USART-Ports
//		uint8_t end_of_string -> Zeichenkettenende bis zu welchem die Bytes empfangen werden sollen.
//									Maximale Anzahl wird duch den FIFO bestimmt.
////////////////////////////////////////////////////////////////////////////////////////////////////
void USART_set_end_of_string(uint8_t USARTnumber, uint8_t end_of_string)
{
	USART_end_of_string[USARTnumber] = end_of_string;
}
