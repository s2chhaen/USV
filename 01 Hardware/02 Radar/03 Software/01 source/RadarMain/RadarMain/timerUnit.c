/*
 * timerUnit.c
 * Description: Quellcode-Datei der timerUnit.h
...
static uint16_t radarTimer_res = 0;
static uint16_t usvTimer_res = 0;
volatile int16_t radarTimer_stepCounter = 0;
 ...
 * \param resRadar Auflösung für Sensor-Handler-Wächter
 * 
 * \return uint8_t 0: kein Fehler, sonst: Fehler
 */
uint8_t timer_init(uint8_t rezConfig, uint16_t resUSV, uint16_t resRadar){
	...
	switch(rezConfig){
		case REZ_US:
			...
			resRadar = (resRadar < TIMER_MIN_REZ_US)?TIMER_MIN_REZ_US:resRadar;
			break;
		case REZ_MS:
			...
			resRadar = (resRadar > TIMER_MAX_REZ_MS)?TIMER_MAX_REZ_MS:resRadar;
			break;
		case REZ_S:
			config |= TCA_SINGLE_CLKSEL_DIV1024_gc;
			prescalerWConvertFactor = 1024UL;
			resUSV = (resUSV > TIMER_MAX_REZ_S)?TIMER_MAX_REZ_S:resUSV;
			resRadar = (resRadar > TIMER_MAX_REZ_S)?TIMER_MAX_REZ_S:resRadar;
			break;
		default:
			result = PROCESS_FAIL;
			break;
	...
	if (result==NO_ERROR){
		...
		radarTimer_res = resRadar;
		temp = (uint16_t)(CLK_CPU*10/prescalerWConvertFactor*resRadar/10);
		TCB1.CCMP = temp;
...
//Funktion zum Steuern des Timers für Radar
/**
 * \brief Ein-/Ausschalten des Radar-Handler-Wächters
 * 
 * \param state 0: Ausschalten, sonst Einschalten
 * 
 * \return void
 */
void radarTimer_setState(uint8_t state){
	if (timer_status.init){
		if (state){
			TCB1.CTRLA |= TCB_ENABLE_bm;
		} else{
 ...
 * \return void
 */
void radarTimer_setCounter(uint32_t value){
	radarTimer_stepCounter = (int16_t)(value/(uint32_t)radarTimer_res);
	TCB1.CNT = 0;
}
 ...
 * \return const int16_t die übrige Zeit in Zählschritte
 */
const int16_t radarTimer_getCounter(){
	return radarTimer_stepCounter;
}
